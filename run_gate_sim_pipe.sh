#!/usr/bin/env bash
# ./run_gate_sim_pipe.sh              # batch, low-effort netlist
# ./run_gate_sim_pipe.sh high         # batch, high-effort netlist
# ./run_gate_sim_pipe.sh low gui      # gui, low-effort netlist
# ./run_gate_sim_pipe.sh high gui     # gui, high-effort netlist
#
...
case "$EFFORT" in
    low|high) NETLIST_DIR="synth_pipe_${EFFORT}" ;;
...
if [[ "$MODE" == "gui" || "$MODE" == "-gui" ]]; then
    ...
    CMD=(xrun "${FLAGS[@]}" "${SRC[@]}" -top "$TOP" -input sim.tcl -l "xrun_gate_pipe_${EFFORT}.log")
fi
