...
module product_normalizer (
  ...
  reg signed  [ 6:0] unrounded_product_exp_reg; 
  reg         [21:0] normalized_mantissa_reg;
  ...
  always @(posedge clk or negedge rst_n) begin                            
    if (!rst_n) begin                                                     
      unrounded_product_exp_reg <= 7'sd0;
      normalized_mantissa_reg   <= 22'd0;
    end else begin                                                        
      unrounded_product_exp_reg <= unrounded_product_exp;
  ...
  assign is_denormal = (unrounded_product_exp_reg <= 0);
  assign denorm_shift_amount = 7'sd1 - unrounded_product_exp_reg;
  assign denorm_shift = is_denormal ? ((denorm_shift_amount > 7'sd21) ? 5'd21 : denorm_shift_amount[4:0]) : 5'd0;

  assign shifted_product_mantissa = normalized_mantissa_reg >> denorm_shift;
  ...
  assign denorm_sticky = |(normalized_mantissa_reg & denorm_shift_mask);

  assign unrounded_product_mantissa_o = shifted_product_mantissa[21:11];

  assign unrounded_product_exp_o = is_denormal ? 6'd0 : unrounded_product_exp_reg[5:0];
